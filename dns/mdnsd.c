...
/* todo
- either change the conflict callback to also notify when it's passed, or have an automatic way of pushing out records associated with it being ok
- better comments in examples, mention timing
- api overhaul, handle one-shot queries better? expose the cached/incoming results as "unique" or not (based on flush bit)?
*/
            ...
            { // check all of our potential answers
                ...
                { // probing state, check for conflicts
                    ...
                    { // check all to-be answers against our own
                        if((m->qd[i].type != 255 && m->qd[i].type != m->an[j].type) || strcmp(m->qd[i].name,m->an[j].name)) continue;
